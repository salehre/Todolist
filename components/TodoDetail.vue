<template>
  <div :class="[
    'rounded-2xl border border-pink-200/50 bg-white/30 shadow-lg shadow-pink-200/40 backdrop-blur-sm flex flex-col h-full overflow-hidden',
    ...
    <div v-if="modelValue" class="h-full flex flex-col">
      ...
      <!-- Fixed Header -->
      <div class="shrink-0 bg-white/95 backdrop-blur-md border-b border-pink-200 shadow-sm z-10">
        <div class="max-w-4xl mx-auto px-4 md:px-4 py-4">
          ...
          modelValue.completed ? 'bg-pink-50 text-pink-600' : 'bg-orange-50 text-orange-600'
        ]">
          ...
          {{ modelValue.completed ? 'Completed' : 'Pending ..' }}
            ...
            <div class="flex gap-3">
              <button
                  ...
                  class="px-2 md:px-4 py-2 bg-pink-600 hover:bg-pink-700 text-white rounded-sm font-medium transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
              >
              ...
              <button
                  ...
                  class="px-2 md:px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded-sm font-medium transition-all flex items-center gap-2"
              >
              ...
              <button
                  @click="emit('back')"
                  :class="['flex items-center gap-1 px-2 py-2 bg-pink-50 text-pink-700 rounded-lg text-sm font-medium hover:bg-pink-100 transition-all shrink-0', !showBackButton && 'sm:hidden']"
              >
        ...
        <div class="p-4 md:p-6">
          <div class="max-w-4xl mx-auto">
            <div class="bg-white rounded-2xl shadow-xl border border-pink-100 overflow-hidden">
              ...
              <!-- Title Section -->
              <div class="p-6 md:p-8 border-b border-pink-100 bg-linear-to-r from-pink-50 to-white">
                <div class="flex items-start justify-between">
                  ...
                  <div class="pt-1.5 flex items-center px-3 shrink-0">
                    <input
                        ...
                        class=" w-4 h-4 md:w-5 md:h-5 accent-red-600 cursor-pointer"
                    />
                  </div>
                  <div class="flex-1">
                    <div class="flex items-center gap-3 mb-3">
                      <span :class="[
                        ...
                        modelValue.completed ? 'line-through text-pink-400' : 'text-purple-950'
                      ]">
                        {{ modelValue.text }}
                      </span>
                    </div>
              ...
              <div class="p-6 md:p-8">
                <div class="flex items-center gap-2 mb-4">
                  <Icon class="text-xl mb-2 text-rose-800" icon="ic:outline-description" />
                  <h3 class="text-lg font-semibold text-rose-800">Description</h3>
                </div>
                <div v-if="modelValue.description" class="bg-pink-50 rounded-xl p-4 md:p-6 text-gray-700 whitespace-pre-wrap leading-relaxed wrap-break-word">
                  {{ modelValue.description }}
                </div>
                <div v-else class="bg-pink-50 rounded-xl p-4 md:p-6 text-pink-400 text-center italic">
                  No description provided for this task.
                </div>
              </div>

              <!-- Progress Path Section -->
              <div class="p-6 md:p-8 bg-pink-50/30 border-t border-pink-100">
                <div class="flex items-center justify-between mb-6">
                  <div class="flex items-center gap-2">
                    <span class="text-xl"><Icon icon="ci:path" class="text-red-800 mb-1" /></span>
                    <h3 class="text-lg font-semibold text-red-700">Progress Path</h3>
                    <span v-if="modelValue.steps && modelValue.steps.length > 0" class="text-sm text-pink-500 font-medium">
                  ...
                  <div class="flex gap-2">
                    <button
                        ...
                        class="px-3 py-2 bg-red-100 text-red-600 rounded-sm text-sm font-medium hover:bg-red-200 transition-all flex items-center gap-1"
                    >
                    ...
                    <button
                        ...
                        class="px-3 md:px-4 py-2 bg-pink-600 text-white rounded-sm text-sm font-medium hover:bg-pink-700 transition-all flex items-center gap-1"
                    >
                ...
                <div v-if="modelValue.steps && modelValue.steps.length > 0" class="relative">
                  <!-- Connection Line -->
                  <div class="absolute inset-s-5 md:inset-s-7.5 top-0 bottom-0 w-0.5 bg-pink-200">
                    <div
                        class="w-full bg-linear-to-b from-pink-500 to-rose-500 transition-all duration-500"
                    ...
                    >
                      ...
                      <div class="relative inset-s-1.5 md:inset-s-2.25 z-10 shrink-0">
                        <div :class="[
                          ...
                          step.completed
                            ? 'bg-linear-to-br from-pink-500 to-rose-500 border-pink-600 text-white shadow-lg shadow-pink-200'
                      ...
                      <div :class="[
                        ...
                        step.completed
                          ? 'bg-linear-to-r from-pink-50 to-white border-pink-200'
                          : index === getCurrentIncompleteIndex(modelValue)
                          ? 'bg-white border-pink-500 shadow-md'
                          : 'bg-white border-pink-100 opacity-60'
                      ]">
                        <div class="flex items-center justify-between gap-2 md:gap-3">
                          <div class="flex-1">
                            ...
                            <p class="text-xs mt-1">
                              ...
                              <span v-else class="text-red-900"> Waiting for previous steps</span>
                            </p>
                          </div>
                          <div class="flex items-center gap-2 shrink-0">
                            <input
                                ...
                                class="w-4 h-4 md:w-5 md:h-5 accent-pink-600 cursor-pointer disabled:opacity-30 disabled:cursor-not-allowed"
                            />
                            <button
                ...
                <div v-else class="flex flex-col items-center py-8">
                  <div class="text-4xl text-pink-400 mb-3"><Icon icon="ci:path" /></div>
                  <p class="text-pink-400 text-sm tracking-wide"> create a path for your task</p>
                </div>
              </div>

              <!-- Footer: Date -->
              <div class="px-6 md:px-8 py-3 border-t border-pink-100 bg-pink-50/40 flex items-center justify-end gap-1.5 text-xs text-pink-500">
    ...
    <div v-if="showClearStepsDialog" class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" @click.self="showClearStepsDialog = false">
      <div class="bg-white rounded-2xl shadow-2xl max-w-sm w-full">
        <div class="p-6 flex flex-col items-center text-center">
          <div class="text-5xl mb-2"><Icon class="text-red-900" icon="quill:folder-trash" /></div>
          <h3 class="text-xl font-bold text-red-800 mb-2">Clear All Steps</h3>
          <p class="text-red-600 text-sm mb-2">Are you sure you want to clear all steps?</p>
          <!--          <p class="text-pink-400 text-xs">This action cannot be undone!</p>-->
        </div>
        <div class="flex gap-3 p-6 border-t border-pink-100 bg-pink-50/50 rounded-b-2xl">
          <button @click="showClearStepsDialog = false" class="flex-1 px-4 py-2 bg-white border border-pink-200 text-rose-700 rounded-xl font-medium hover:bg-pink-50 transition-all">Cancel</button>
          <button @click="confirmClearSteps" class="flex-1 px-4 py-2 bg-red-700 hover:bg-red-800 text-white rounded-xl font-medium transition-all">Clear All</button>
        </div>
      </div>
    </div>
    ...
    <div v-if="showStepsDialog" class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" @click.self="closeStepsDialog">
      <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full max-h-[90vh] flex flex-col">
        <div class="flex flex-col px-6 border-b border-pink-100">
          <button @click="closeStepsDialog" class="text-pink-400 relative top-4 left-48 hover:text-pink-600 text-2xl">✕</button>
        ...
        <!-- Preview -->
        <div class="p-6 bg-pink-50/50 border-b border-pink-100">
          <div class="text-xs text-rose-600 mb-3 font-medium">PREVIEW</div>
          <div class="flex items-center gap-2 flex-wrap">
            <template v-for="(step, index) in stepsDraft" :key="step.id">
              <div class="flex items-center gap-1">
                <div :class="[
                  'w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold',
                  index === 0 ? 'bg-pink-500 text-white' : 'bg-pink-200 text-pink-600'
                ]">{{ index + 1 }}</div>
                <span class="text-xs text-rose-700 truncate max-w-25">{{ step.text || `Step ${index + 1}` }}</span>
              </div>
              <div v-if="index < stepsDraft.length - 1" class="w-4 h-0.5 bg-pink-300"></div>
            </template>
          </div>
        </div>
        ...
        <div class="flex-1 overflow-y-auto p-6 space-y-3">
          <div v-for="(step, index) in stepsDraft" :key="step.id" class="flex items-center gap-2">
            <div class="w-8 h-8 rounded-full bg-pink-100 flex items-center justify-center text-sm font-bold text-pink-600 shrink-0">
              {{ index + 1 }}
            </div>
            <input
                v-model="step.text"
                ...
                class="flex-1 px-3 py-2 rounded-lg border border-pink-200 focus:border-pink-500 focus:ring-2 focus:ring-pink-200 focus:outline-none text-sm"
            />
            <button @click="removeStepFromDraft(index)" class="text-red-400 hover:text-red-600 transition-all shrink-0">
              <Icon icon="mingcute:delete-line" class="text-lg" />
            </button>
          </div>
          <button
              @click="addStepToDraft"
              class="w-full py-2 border-2 border-dashed border-pink-300 rounded-xl text-pink-500 hover:bg-pink-50 transition-all text-sm flex items-center justify-center gap-1"
          >
        ...
        <div class="flex gap-3 p-6 border-t border-pink-100 bg-pink-50/50 rounded-b-2xl">
          <button @click="closeStepsDialog" class="flex-1 px-4 py-2 bg-white border border-pink-200 text-rose-700 rounded-xl font-medium hover:bg-pink-50 transition-all">Cancel</button>
          <button @click="saveSteps" class="flex-1 px-4 py-2 bg-linear-to-r from-pink-500 to-rose-500 text-white rounded-xl font-medium hover:from-pink-600 hover:to-rose-600 transition-all">Save Steps</button>
        </div>
      </div>
    </div>
