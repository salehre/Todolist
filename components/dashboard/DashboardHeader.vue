            ...
            >
              ...
              >
                ...
                <div class="min-w-0 flex-1">
                  ...
                  <p class="text-[11px] text-primary-500">{{ notice.count }} پیام جدید</p>
                </div>
                <span class="shrink-0 flex h-5 min-w-5 items-center justify-center rounded-full bg-primary-500 px-1 text-[10px] font-bold text-white">
                  {{ notice.count > 99 ? '99+' : notice.count }}
                </span>
              </button>
...
const { bumpUnreadFromNotification, markMentionedFromNotification, markReactedFromNotification } = useGroupChat()
const { getActiveGroupId } = useGroupChat()
const { fetchGroups, groups } = useGroupChat()
...
onMounted(() => {
  ...
  if (authState.user?.id) {
    ...
    $echo.private(`user.${authState.user.id}`)
        ...
        .listen('.message.notification', (e: { groupId: number; groupName: string; isMentionOrReply: boolean }) => {
          if (e.groupId === getActiveGroupId()) return
          addMessageNotice(e.groupId, e.groupName)
          bumpUnreadFromNotification(e.groupId)
          if (e.isMentionOrReply) markMentionedFromNotification(e.groupId)
        })
        .listen('.reaction.notification', (e: { groupId: number; groupName: string }) => {
          if (e.groupId === getActiveGroupId()) return
          markReactedFromNotification(e.groupId)
        })
