  ...
  <div class="flex h-full overflow-hidden font-sans relative">
    ...
    <div v-if="isDialogOpen" class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" @click.self="closeDialog">
      <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        <div class="flex justify-between items-center p-6 border-b border-pink-100 sticky top-0 bg-white z-10">
          <h3 class="text-xl font-bold text-rose-900">
            {{ dialogMode === 'add' ? '➕ Add New Task' : '✏️ Edit Task' }}
          </h3>
          <button @click="closeDialog" class="text-pink-400 hover:text-pink-600 text-2xl">✕</button>
        </div>
        <div class="p-6 space-y-4">
          ...
          <div>
            <label class="block text-sm font-medium text-rose-700 mb-2">Description</label>
            <textarea
          ...
          <div>
            <label class="block text-sm font-medium text-rose-700 mb-2">Priority</label>
            <select
        ...
        <div class="flex gap-3 p-6 border-t border-pink-100 rounded-b-2xl sticky bottom-0 bg-white">
          <button @click="closeDialog" class="flex-1 px-4 py-2 bg-white border border-pink-200 text-rose-700 rounded-xl font-medium hover:bg-pink-50 transition-all">Cancel</button>
          <button
              @click="submitDialog"
    ...
    <div v-if="showDeleteDialog" class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" @click.self="closeDeleteDialog">
      <div class="bg-white rounded-2xl shadow-2xl max-w-sm w-full">
        <div class="p-6 text-center">
          ...
          <p class="text-rose-600 text-sm mb-2">Are you sure you want to delete this task?</p>
        ...
        <div class="flex gap-3 p-6 border-t border-pink-100 bg-pink-50/50 rounded-b-2xl">
          <button @click="closeDeleteDialog" class="flex-1 px-4 py-2 bg-white border border-pink-200 text-rose-700 rounded-xl font-medium hover:bg-pink-50 transition-all">Cancel</button>
          <button @click="confirmDelete" class="flex-1 px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded-xl font-medium transition-all">Delete</button>
        </div>
      </div>
    </div>
